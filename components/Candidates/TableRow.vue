...
<template>
  <tr class="text-base bg-gray-50 border-b max-xl:text-sm">
    ...
    <td id="button-open" scope="row" class="flex items-center px-3 py-5 text-gray-900 cursor-pointer">
    ...
    <td v-if="tableTdVisible.isRatingVisible" scope="row" class="px-3 py-4">
      <button class="flex items-center gap-1 border px-2 py-[.05rem] rounded-full">
        <SolidStartIcon v-if="rating > 0" class="text-yellow-300 w-4 h-4" />
        <StarIcon v-else class="text-gray-300 w-4 h-4" />
        <span :class="`${rating > 0 ? 'text-gray-800 font-semibold' : 'text-gray-500'
          } text-sm`">{{ rating }}.0</span>
      </button>
    </td>
